...
export default function ActivityList({activities, dispatch} : ActivityListProps) {
    ...
    ,[activities])
    const isEmptyActivityList = useMemo(() => activities.length === 0, [activities]) ;
  return (
    <>
        <h2 className="text-4xl font-bold text-slate-600 text-center">Comida y actividades</h2>

        { isEmptyActivityList ? 
            <p className="text-center my-6">No hay actividades aún...</p> :
        activities.map(activity=> (
            <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow-sm">
                <div className="space-y-2 relative">
                    ...
                    <p className="font-black text-lime-500">
                        {activity.calories} {''}
                        <span>Calorías</span>
                    </p>
                </div>
